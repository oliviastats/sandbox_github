...
data_path_gcs = (
    "gs://mgb-transformers-sandbox-dev-data/olivia_test/kaggle/insurance.csv"
)
...
def get_model_evaluation_metrics(y_pred: np.array):
    ...
    r_squared_adjusted = 1 - (
        (1 - r_squared) * (len(y_test) - 1) / (len(y_test) - X_train.shape[1] - 1)
    )
...
if __name__ == "__main__":
    ...
    # data preprocessing
    insurance_df_one_hot_encoded = get_dummies_for_categorical(
        insurance_df, ["sex", "children", "smoker", "region"]
    )
    X_train, X_test, y_train, y_test = get_train_test_split(
        0.3, insurance_df_one_hot_encoded
    )
    ...
    (
        mean_squared_error_lin_reg,
        r_squared_lin_reg,
        r_squared_adjusted_lin_reg,
    ) = get_model_evaluation_metrics(y_pred_lin_reg)

    # k-fold CV (using all independent variables)
    scores_r_squared_lin_reg = cross_validate(
        linear_regressor, X_train, y_train, scoring="r2", cv=5, return_train_score=True
    )
    # test scores
    ...
    (
        ...
        r_squared_ridge,
        r_squared_adjusted_ridge,
    ) = get_model_evaluation_metrics(y_pred_ridge)

    # k-fold CV (using all independent variables)
    scores_r_squared_ridge = cross_validate(
        ridge_regression, X_train, y_train, scoring="r2", cv=5, return_train_score=True
    )
    ...
    (
        ...
        r_squared_lasso,
        r_squared_adjusted_lasso,
    ) = get_model_evaluation_metrics(y_pred_lasso)

    # k-fold CV (using all independent variables)
    scores_r_squared_lasso = cross_validate(
        lasso_regression, X_train, y_train, scoring="r2", cv=5, return_train_score=True
    )
    # test scores for ridge

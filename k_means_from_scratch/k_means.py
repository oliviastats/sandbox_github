...
class kmeans:
    ...
    def recompute_centroids(self, X, centroids, assigned_clusters):
        ...
        for centroid in centroids:
            ...
            X_same_cluster = X[
                np.logical_and(cond_same_cluster[:, 0], cond_same_cluster[:, 1])
            ]
    ...
    def run_kmeans(self, k, X):
        ...
        while centroid_dist_change > 0.001:
            assigned_cluster = self.assign_cluster(X, centroids_prev)
            centroids_new = self.recompute_centroids(
                X, centroids_prev, assigned_cluster
            )
            centroid_dist_change = self.euclidean_distance(
                centroids_new, centroids_prev
            )
